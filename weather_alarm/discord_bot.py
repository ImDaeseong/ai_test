...
class DiscordWeatherBot(discord.Client):
    def __init__(self, weather_client: WeatherClient, channel_id: int = 0):
    ...
    def _register_commands(self):
        ...
        async def _gasanweather(interaction: discord.Interaction):
            ...
            data: WeatherData | None = await self.weather_client.fetch()
    ...
    async def on_ready(self):
        ...
        if not self.channel_id:
            logger.warning("DISCORD_CHANNEL_ID가 설정되지 않아 시작 알림을 건너뜁니다.")
            return
        try:
            channel = await self.fetch_channel(self.channel_id)
        except Exception as e:
            logger.warning(f"Discord 채널을 가져올 수 없습니다: channel_id={self.channel_id}, 오류={e}")
            return
        data: WeatherData | None = await self.weather_client.fetch()
        if data is None:
            logger.warning("Discord 시작 알림: 날씨 데이터 조회 실패")
            return
        date_str = f"{data.base_date[:4]}-{data.base_date[4:6]}-{data.base_date[6:]}"
        time_str = f"{data.base_time[:2]}:{data.base_time[2:]}"
        embed = discord.Embed(

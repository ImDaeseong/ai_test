...
| 파일 | 내용 |
|---|---|
| find_string_app.py | 창 닫기 시 after() 콜백이 무한 반복되던 문제 수정 — _on_close() 메서드 추가 및 _polling 플래그로 제어 |
| find_string_app.py | 재검색 시 이전 검색 잔여 메시지가 새 결과와 혼재되던 문제 수정 — outbox 큐 drain 추가 |
| find_string_app.py | self.matches 리스트가 사용되지 않으면서 메모리를 계속 누적하던 문제 제거 |

...
> **최종 수정**: 2026-05-16 (곡 분위기 기반 무드/시각 단서 추론을 `configs/song_inference.json` 프로필 구조로 분리)  
...
|----------|------|--------|-----------|
| `cyber_noir` | graphite gray | icy cyan | silver white |
...
5. lighting_language() - BPM 기반 조명 설명 + 프로필 texture + 스타일별 shadow/highlight
   ...
   └─ BRAND_PALETTE["shadow_color"]: 스타일별 그림자 색상 사용
      예: warm_acoustic → "deep earth shadow and dark mahogany"
          cyber_noir   → "deep plum and dark violet"
...
6. transition_language() - BPM 기반 전환 설명 + 스타일별 보조 강조색
   └─ BRAND_PALETTE["secondary_accent"]: 스타일별 반사 색상 사용
      예: warm_acoustic → "candlelight reflections"
          cyber_noir   → "icy cyan reflections"
...
④ 가사 분위기                        scene.lyric_visual_idea (앞의 "lyric cue:" 제거, 최대 220자)
   "Lyric mood: {lyric_idea}."
...
| `configs/visual_styles.json` | **7종** 비주얼 스타일 정의 (팔레트, 단계별 색상 비율, 전역 애니메이션 제약) | 새 스타일 추가, 기존 팔레트 색상 변경 |
...
<select name="style_id">
  ...
  <option value="cyber_noir">Cyber Noir</option>
...
scene_generator.py
  ...
  generate_scenes()        → 씬별 6가지 요소 결정 (조명도 색상 치환)
  apply_story_arc_to_scenes() → 씬별 story_stage/story_beat 추가
  image_prompt()           → scene.image_prompt (10개 요소 조립)
  video_prompt()           → scene.video_prompt (10개 요소 조립)

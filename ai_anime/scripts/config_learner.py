...
_STRIP_PREFIX = re.compile(r"^(and|plus|or|with|a|an|the)\s+", re.I)
# Tags that are recording/production specs rather than genre/style identifiers
_PRODUCTION_RE = re.compile(
    r"\b(bpm|\d+hz|\d+\s*db|\d+.bit|khz|stereo|mono|eq|reverb|compress"
    r"|saturat|tape|vinyl|crackle|channel|sample\s+rate|time\s+signature"
    r"|breathin|breathiness|whisper|half.whisper|high.fidel|analog"
    r"|arpegg|staccato|syncopat|offbeat|kick|snare|hat\b|arpeggio"
    r"|panned|panning|filter|build|transition|loop\b|organic\s+drum"
    r"|driving\s+bass|vocal\s+rhythm|vocal\s+layering|lead\s+bridge"
    r"|\bpads\b|\bstabs\b|\bchops\b|\bchopped\b)"
    r"|\b\d{2,3}\s*bpm\b|\b\d+/\d+\b",
    re.I,
)
...
def dedupe_by_url(entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Keep only the most-recent entry per Suno URL."""
    seen: dict[str, dict[str, Any]] = {}
    for entry in entries:
        url = entry.get("url", "")
        if url not in seen or entry.get("timestamp", "") > seen[url].get("timestamp", ""):
            seen[url] = entry
...
def run(min_freq: int = 1, dry_run: bool = False) -> dict[str, Any]:
    """
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="config에 추가할 최소 등장 횟수 (기본값: 1)",
    )
    args = parser.parse_args()
    report = run(min_freq=args.min_freq, dry_run=args.dry_run)

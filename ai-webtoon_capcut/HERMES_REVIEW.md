...
> 변경되지 않았다. 원문은 그대로 남기되 체크 표시만 코드 기준으로 정정했다.
...
- `scripts/test.ps1`: 30개 테스트 통과 (2026-06-06 당시 기록; 2026-08-17 재확인 결과 현재 14개)
...
## 렌더·정렬 추가 검증 (2026-08-17: 현재 코드로 재현 불가, 원문 보존)

아래는 2026-06-06 시점 원문이다. 현재 저장소에는 이 결과를 만들어낼 렌더러·정렬 코드가
없으므로 사실로 취급하지 않는다.

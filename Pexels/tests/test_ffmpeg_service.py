...
def test_build_youtube_export_command_uses_upload_friendly_codecs():
    ...
    assert command[0] == "ffmpeg"
    # Video is copied (already encoded by trim_scene) — no re-encoding
    assert "copy" in command
    assert "libx264" not in command
    # Audio encoding
    assert "aac" in command
    assert "48000" in command
    # Subtitle and container
...
def test_trim_audio_command(monkeypatch, tmp_path: Path):
    calls = []
    monkeypatch.setattr("app.services.ffmpeg_service.FFmpegService.ensure_ffmpeg", lambda _: None)
    ...
    assert "-vn" in command  # must exclude embedded album art / video streams
    assert "aac" in command
    assert "48000" in command
...
def test_cleanup_processed_for_output(tmp_path: Path):

...
| 파일 | 내용 |
|---|---|
| app/services/ffmpeg_service.py:77 | trim_scene subprocess 인코딩 미지정 수정 (encoding='utf-8', errors='replace' 추가) |
| app/services/ffmpeg_service.py:104 | concat 1차 실패 시 오류 정보가 소실되던 문제 수정 — logger.warning으로 stderr 기록 후 재시도 |
| app/services/ffmpeg_service.py:1 | import logging 및 logger = logging.getLogger(__name__) 추가 |

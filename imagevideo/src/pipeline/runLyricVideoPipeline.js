...
async function main() {
  ...
  try {
    ...
    const context = {...inputFiles, debug: args.debug, title: args.title, artist: args.artist};
...
function printHelp() {
  console.log([
    ...
    '  background image:    optional .png, .jpg, .jpeg, or .webp under input/',
    '  background video:    optional .mp4, .mov, .webm under input/ (loops)',
    `  typography video:    ${FILES.typographyVideo}`,
    `  final video:         ${FILES.finalVideo}`,
    '',
    'Options:',
    '  --audio <path>      Override input/audio.mp3 or input/audio.wav.',
    '  --title <text>      Song title shown as intro overlay (first 4 seconds).',
    '  --artist <text>     Artist name shown under title in intro overlay.',
    '  --clean             Remove standard old outputs and logs before running.',
    '  --debug             Print full phase commands.',
    '  --skip-plan         Do not regenerate output/production_plan.json.',
    '  --skip-subtitles    Do not regenerate output/subtitles.ass.',
    '  --skip-motion       Do not render output/rendered_typography_video.mp4.',
    '  --skip-compose      Do not run FFmpeg composition.',

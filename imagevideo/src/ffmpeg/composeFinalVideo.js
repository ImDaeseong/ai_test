...
function buildTitleChain(title, artist, duration) {
  ...
  if (title) {
    ...
    filters.push(
      `drawtext=text='${escaped}':fontsize=52:fontcolor=white:x=(W-text_w)/2:y=H*0.10:shadowx=3:shadowy=3:shadowcolor=black@0.90:alpha='${alphaExpr}':enable='${enableExpr}'`
    );
  }
  ...
  if (artist) {
    ...
    filters.push(
      `drawtext=text='${escaped}':fontsize=30:fontcolor=0xDDDDDD:x=(W-text_w)/2:y=H*0.10+68:shadowx=2:shadowy=2:shadowcolor=black@0.90:alpha='${alphaExpr}':enable='${enableExpr}'`
    );
  }

...
- **4단계 파이프라인**: plan → subtitles → render → compose
  ...
  3. **타이포그래피 렌더링**: FFmpeg 줌/팬 모션, 배경 영상 루프, 어두운 오버레이, 비네트, 곡물 노이즈
  4. **최종 합성**: 오디오 + 인트로 타이틀/아티스트 오버레이 + H.264/AAC MP4 출력
...
npm run lyric-video -- --clean --title "곡명" --artist "아티스트"
...
### 기술 스택
- Node.js 20+ / ESM 모듈
- FFmpeg / ffprobe (시스템 설치 필요)
- Motion Canvas 3.17.2 (실험적 경로)

### 개발 완성도: ★★★★☆
4단계 FFmpeg 파이프라인 동작 확인 (output/ 폴더에 lyric_video.mp4 생성). 배경 이미지/영상 자동 감지, 단계별 로그, 제목/아티스트 인트로 옵션이 반영됨. Motion Canvas 경로는 씬 생성과 개발 미리보기까지 준비되어 있으나, 설치 패키지의 안정적인 비대화형 렌더 CLI가 없어 실험 경로로 분리되어 있음.
...
- **Pexels 영상 검색**: 장면별 키워드로 스톡 영상을 검색하고 적합한 MP4 선택
- **다운로드 캐시**: 선택된 Pexels MP4를 다운로드하고 기존 캐시 파일 재사용
- **FFmpeg 렌더링**: 클립 트림·스케일·크롭·연결, 음악 길이에 맞춘 최종 영상 생성
...
- **안전 처리**: 외부 API 타임아웃/재시도, Gemini JSON 검증, FFmpeg 인자 리스트 호출로 shell injection 방지
...
### 기술 스택
- Python / Pydantic 2
- Gemini API, Pexels API, httpx
- FFmpeg (PATH 설치 필요)
- pytest 기반 서비스 테스트

### 개발 완성도: ★★★★☆
실행 스크립트, 테스트, 아키텍처 문서, 보안 메모까지 포함된 구조화된 프로젝트. `.env`에 Gemini/Pexels API 키를 설정하고 FFmpeg를 설치하면 `run.bat`으로 테스트 후 최종 MP4와 HTML 리포트를 생성할 수 있음.
...
### 기능 개요
Remotion 프레임워크(React 기반 비디오 생성)를 사용하여 LRC/SRT 가사 파일과 오디오를 조합해 가사 비디오(MP4)를 프로그래밍 방식으로 렌더링하는 프로젝트.

### 주요 기능
- **가사 동기화 시각화**: 현재/이전/다음 3줄 동시 표시, 페이드 인/아웃 애니메이션
- **음파형 시각화**: SVG 기반 실시간 음파형 (스파이크 바 + 라인 웨이브 + 파티클)
- **인트로/아웃트로**: 곡명·아티스트 오버레이 페이드, 검은 페이드아웃
- **배경 유연성**: 이미지(줌 애니메이션) / 비디오 / 그라디언트 기본 배경
- **이중 포맷**: 16:9(1920×1080, YouTube) / 9:16(1080×1920, Shorts/Reels/TikTok)
- **자동 매니페스트**: `public/media/` 폴더 파일 자동 감지 스크립트
...
### 기능 개요
Windows 로컬 시스템의 TCP/UDP 네트워크 연결을 실시간으로 모니터링하고, Raw Socket 패킷 캡처로 HTTP 요청 및 TLS SNI를 추출하는 보안·진단 도구.

### 주요 기능
- **연결 모니터링**: psutil로 0.5초 주기 TCP/UDP 연결 추적, 프로세스별 LISTEN/ESTABLISHED 매핑
- **HTTP 패킷 캡처**: Raw Socket으로 실제 HTTP 요청 URL, 메서드, Host 추출
- **TLS SNI 추출**: TLS ClientHello 분석으로 HTTPS 도메인명 식별 (암호화 전)
- **웹 프로토콜 탐지**: LISTEN 포트에 직접 HTTP/HTTPS HEAD 요청 (선택)
- **Reverse DNS**: 백그라운드 스레드풀로 원격 IP → 도메인명 조회
- **로그 출력**: JSON 스냅샷 (`network_log.json`) + JSONL append + 로테이션 텍스트 로그
- **프로세스 필터**: `-p python` 등 특정 프로세스만 표시 가능

### 폴더 구조
```
port_scan/
├── network_monitor.py       # 메인 (949줄) - 수집·스니퍼·DNS·출력·로그
├── test_network_monitor.py  # 유닛 테스트
└── run_monitor.bat          # 실행 (관리자 권한 자동 상승)
```

### 사용 방법
```bash
# 배치 파일 실행 (권장 - 관리자 권한 자동 상승)
run_monitor.bat

# 직접 실행 (관리자 권한 필요)
python network_monitor.py
python network_monitor.py -p chrome -p node --active-probe --display-interval 3
python network_monitor.py --no-sniffer --no-rdns --jsonl
```

### CLI 주요 옵션
| 옵션 | 설명 |
|------|------|
| `-p NAME` | 프로세스 이름 필터 (다중) |
| `--active-probe` | HTTP/HTTPS 능동 탐지 |
| `--no-sniffer` | 패킷 캡처 비활성화 |
| `--no-rdns` | Reverse DNS 비활성화 |
| `--jsonl` | JSONL append 로그 |
| `--redact-url-query` | URL 쿼리 파라미터 마스킹 |

### 기술 스택
- Python 3.8+ / psutil (자동 설치)
- Raw Socket (관리자 권한 필요)

### 개발 완성도: ★★★★☆
모니터링·스니퍼·DNS·로그 기능 완성. 유닛 테스트 포함. Raw Socket은 관리자 권한 필수. Windows 전용 (Raw Socket 구현 방식).

---

## 12. security_scanning
...
- 보안 진단 도구 2종 (port_scan, security_scanning) — 내부망 진단 용도
